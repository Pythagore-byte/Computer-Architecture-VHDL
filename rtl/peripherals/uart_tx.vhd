    ...
    begin
        if reset = '1' then
            ...
            case etat_present is
                ...
                when E2 =>
                    reg     <= '1' & Data & '0'; -- stop + donnee + start
                    cnt_bit <= 0;
...
--                     Tx      <= '1';
--                     cnt_bit <= 0;
--                     -- On attend à la fois l'appui sur GO et un tick d'horloge 
--                     -- pour synchroniser proprement le début de la trame
--                     if Go = '1' and Tick_bit = '1' then
--                         reg          <= Data;
--                         Tx           <= '0'; -- Début immédiat du bit de start sur le tick
--                         etat_present <= START_BIT;
--                     end if;
...
--                     if Tick_bit = '1' then
--                         cnt_bit      <= 0;
--                         Tx           <= reg(0); -- Premier bit de données
--                         etat_present <= DATA_BITS;
--                     end if;

--                 when DATA_BITS =>
--                     if Tick_bit = '1' then
--                         if cnt_bit = 7 then
--                             Tx           <= '1'; -- Passage au Stop bit
--                             etat_present <= STOP_BIT;
--                         else
--                             cnt_bit      <= cnt_bit + 1;
--                             Tx           <= reg(cnt_bit + 1);
--                         end if;
